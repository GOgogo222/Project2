...
到这里，回顾一下list和set容器，list的特点是它的迭代器（只能向左或向右一步一步地移动），而set的特点则是其自带一个从小到大的排序算法；另外，这两个容器在排序算法（algorithm sort）上很有特点，list由于其迭代器的限制，无法使用algorithm的函数，而是“内嵌”有一个排序函数，二者在使用方法上略有不同。。。sert